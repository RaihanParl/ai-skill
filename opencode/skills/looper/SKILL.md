...
allowed-tools: read write edit bash glob grep ls mkdir
---
...
`LOOPER_ROOT` = `/Users/mraihanparlaungan/.config/opencode/skills/looper`

## Workflow

1. Resolve target path from user's "run looper ..." request. If no target given,
   use `./looper-output`. If target contains existing `loop.yaml`, treat as
   edit/resume instead of fresh scaffold.
2. Load relevant rubric only when entering that stage:
   - Goal stage: `$LOOPER_ROOT/references/goal-rubric.md`.
   - Verification stage: `$LOOPER_ROOT/references/verification-rubric.md`.
   - Council stage: `$LOOPER_ROOT/references/council-rubric.md`.
   - Control stage: `$LOOPER_ROOT/references/control-rubric.md`.
   - Model detection or privacy details: `$LOOPER_ROOT/references/model-detection.md`.
...
- Copy `$LOOPER_ROOT/templates/run-loop.py` exactly unless the user explicitly
  asks to edit the external runner contract.

## Helper Scripts

All scripts live at `$LOOPER_ROOT/scripts/looper.py`. Run with:
`python3 "$LOOPER_ROOT/scripts/looper.py" <subcommand> [args]`

- Detect model CLIs:
  `python3 "$LOOPER_ROOT/scripts/looper.py" detect-models --write`
- Register a custom CLI:
  `python3 "$LOOPER_ROOT/scripts/looper.py" register-model <id> --invoke <cmd> [args...]`
- Compile and render:
  `python3 "$LOOPER_ROOT/scripts/looper.py" compile <target>/loop.yaml --out <target>/loop.resolved.json --render <target>/LOOP.md --session-prompt <target>/RUN_IN_SESSION.md`
